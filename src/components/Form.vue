  ...
  <div class="form">
    <form @submit.prevent="onSubmit" action="/header" method="post" class="form-comp">
      <div class="form-group">
        ...
        <v-autocomplete v-model.trim="form_data.where" :items="items"></v-autocomplete>
      ...
      <div class="form-group">
        ...
        <v-autocomplete v-model.trim="form_data.to" :items="items"></v-autocomplete>
...
export default {
  ...
  data() {
    return {
    items: this.$store.getters.allCity,
    menu2: false, 
      form_data: {
        where: "",
        to: "",
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.form_data.where.length <= 2) {
        this.errors.where =
          "Название города должно содержать больше двух символов";
        isValid = false;
      } else {
        this.errors.where = "";
      }
